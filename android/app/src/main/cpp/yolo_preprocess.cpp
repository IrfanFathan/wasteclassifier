...
static cv::Mat yuv420_to_bgr(
    ...
    for (int row = 0; row < height; ++row) {
        ...
        for (int col = 0; col < width; ++col) {
            ...
            const int g = Y - static_cast<int>(0.344136f * U)
                            - static_cast<int>(0.714136f * V);
...
struct YuvPlanes {
    ...
    void release() {
        if (y_raw) env->ReleaseByteArrayElements(j_y, y_raw, JNI_ABORT);
        if (u_raw) env->ReleaseByteArrayElements(j_u, u_raw, JNI_ABORT);
        if (v_raw) env->ReleaseByteArrayElements(j_v, v_raw, JNI_ABORT);
    }
...
Java_com_example_wasteclassifier_OpenCVHelper_preprocessZone(
    ...
    if (!j_result) { LOGE("preprocessZone: NewFloatArray failed"); return nullptr; }
...
Java_com_example_wasteclassifier_OpenCVHelper_preprocessHorizontalZones(
    ...
    const struct { int x; int w; } zones[3] = {
        { 0,          zone_w   },
        { zone_w,     zone_w   },
        { zone_w * 2, zone_w_r },
    };
